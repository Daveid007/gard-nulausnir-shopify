  ...
  return (
    <header className="sticky top-0 z-30 border-b border-[#24313b]/10 bg-[#f7f9fa]/95 backdrop-blur-md">
      ...
      {open && <nav className="border-t border-[#24313b]/10 px-5 py-5 md:hidden">
        {(categoryNav ? [["Hunangskambur", "#honeycomb"], ["Rúllugardínur", "#roller"], ["WINdoûr Single", "#windour-single"], ["WINdoûr Duo", "#windour-duo"], ["Thedoûr Hurðir & Net", "#thedour-doors"]] : [["Safnið", "#collection"], ["Myrkvun", "#honeycomb"], ["Sérlausnir", "#specialist"]]).map(([label, href]) => <a key={label} href={href} onClick={() => setOpen(false)} className="block py-2 text-[11px] uppercase tracking-[.18em]">{label}</a>)}
      </nav>}
    </header>
  );
}
...
export function Footer() {
  return <footer className="mt-16 border-t border-[#24313b]/10 bg-[#eaf1f5] px-5 py-12 md:px-10"><div className="mx-auto flex max-w-[1480px] flex-col justify-between gap-10 md:flex-row"><div><p className="font-serif text-2xl">Gardínulausnir.is</p><p className="mt-3 max-w-xs text-xs leading-relaxed text-[#596872]">Ljósstýring fyrir íslenskar aðstæður. Mæld, hönnuð og sett upp af fólki sem skilur rými.</p></div><div className="grid grid-cols-2 gap-x-16 gap-y-3 text-[10px] uppercase tracking-[.15em] text-[#43515a]"><a href="#collection">Vörur</a><a href="mailto:hallo@gardinulausnir.is">Hafa samband</a><a href="#top">Upp á topp</a></div></div></footer>;
}