...
export function Collection() {
  ...
  const chooseCategory = (category: CollectionCategoryKey) => {
  ...
  return <div className="solmyrkvun-grid min-h-screen bg-[#f7f9fa]">
    ...
    <main id="all-products" className="mx-auto max-w-[1480px] px-5 py-14 md:px-10 md:py-24">
      ...
      <nav data-testid="category-filter" aria-label="Vöruflokkar" className="grid grid-cols-2 border-b border-[#24313b]/10 sm:grid-cols-3 xl:grid-cols-6">
        {categoryDefinitions.map((definition) => (
          <Link
            ...
            onClick={() => chooseCategory(definition.key)}
            className={`min-w-0 break-words border-b border-[#24313b]/10 px-3 py-5 text-center font-serif text-base leading-snug transition-colors hover:bg-[#eaf1f5] sm:py-7 md:text-lg ${selectedCategory === definition.key ? "bg-[#eaf1f5] text-[#24313b]" : "text-[#596872]"}`}
          >
            {definition.label}
          </Link>
        ))}
